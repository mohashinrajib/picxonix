...
(function() {
    ...
    endLevel(false); // Ensure previous animation loop and state are cleared
...
function update_status_bar(stage='update') {
...
function resetGame(cause) {
    ...
    function applyLevel(img, data) {
        ...
        var i, aPos;
        aPos = cellset.placeBalls(window.ls.nBalls);
        for (i = 0; i < window.ls.nBalls; i++)
            window.ls.aBalls.push(new Enemy(aPos[i][0], aPos[i][1], false));
        aPos = cellset.placeWarders(window.ls.nWarders);
    ...
    window.gs.idFrame = requestAnimationFrame(loop);
    }
    ...
    function loop(now) {
        ...
        if (!window.gs.tLastFrame || update(dt)) {
            render();
        ...
        if (bConquer) {
            ...
            const bonus_captured = cellset.conquer(points_check);
            if (bonus_captured[0]){
    ...
    cursorMoveRemainder += dt * (window.gs.speedCursor + window.gs.speedBonus);
    enemyMoveRemainder += dt * ( Math.min(window.ls.speedEnemy - window.ls.speedEnemyReduce));
        var distCursor = Math.floor(cursorMoveRemainder);
        var distEnemy = Math.floor(enemyMoveRemainder);
        cursorMoveRemainder -= distCursor;
        enemyMoveRemainder -= distEnemy;
    ...
    function unlock() {
        ...
        var aPos = cellset.placeWarders(window.ls.nWarders);
       ...
       _isCollision: function(x, y, dir) {
            ...
            for (var i = 0; i < 4; i++) {
                var d = (dir + aDirs[i] + 360) % 360,
                    vec = dirset.get(d);
                if (cellset.value(x + vec[0], y + vec[1]) & CA_TRAIL)
                    return true;
            }
            return false;
        }
